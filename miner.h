...
#define ASIC_PARSE_COMMANDS(DRIVER_ADD_COMMAND) \
	...
	DRIVER_ADD_COMMAND(bitmineB11) \
	DRIVER_ADD_COMMAND(bitmineB11S) \
...
#define MAX_BOARDS 6
#ifndef  B11_PLL_CLOCK_EN
#define B11_PLL_CLOCK_EN 
#endif
...
#ifndef A1_TEST_MODE_EN
#define A1_TEST_MODE_EN
#endif

#ifndef A1_CHK_CMD_TM
#define A1_CHK_CMD_TM 
#endif


typedef struct _tty_ctx
{
    int fd;
    int baud;
    char devname[20];
...
{
	uint8_t led_val;
	struct timeval last_flash_t;
	struct timeval last_get_nonce_t;
    struct timeval sendlast48_t;

    struct cgpu_info *cgpu;
    struct thr_info set_work_thr;
    int num_chips;
    int num_cores;
//    int count;
    int64_t accept_time;	
#ifdef B11_PLL_CLOCK_EN
    uint16_t regPll;
#endif
    tty_ctx * ctx;
    struct tty_chip *chips;
    ...
    uint32_t FailCount; //count of failed connect to server..

    struct work_queue active_wq;
#ifdef B11_CHK_CMD_TM
    ...
    int shutdown;
#endif
#ifdef B11_TEST_MODE_EN
    uint32_t status;
#endif
    enum   d12_status status;
    int            invalid_nonce_cnt;
    int            invalid_jobid_cnt;
    int            invalid_chipid_cnt;
	int send_cnt;
};
