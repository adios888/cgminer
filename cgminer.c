...
#define WORK_ITEMSIZE sizeof(struct work_item)

struct job_item
{
	int status;
	struct work_ent *data;
};
#define JOB_ITEMSIZE sizeof(struct job_item)


struct work_item workitems[WORK_COUNT];
struct job_item jobitems[WORK_COUNT*3];
...
#endif
#ifdef USE_BITMINE_D12
char *opt_bitmine_d12_options = NULL;
#endif
...
void generate_work()
{
	...
	memset(workitems,0,WORK_ITEMSIZE*WORK_COUNT);	
	for(i=0; i<WORK_COUNT; i++)
	{
		...
		if (work != NULL)
		{
			workitems[i].workid = work->id;
			workitems[i].data = work;
		}
	}
}

inline struct work *pick_work()
{
	int i;
	struct work *work = NULL;
	...
	for(i=0; i<WORK_COUNT; i++)
	{
		if (!workitems[i].status && workitems[i].workid > 0)
		{
			workitems[i].status = 1;
			//printf("pick_work workid = %d\n",workitems[i].workid);
			return workitems[i].data;
		}
	}
...
inline void empty_work(struct work *work)
{
	int i;

	for(i=0; i<WORK_COUNT; i++)
	{
		if (workitems[i].status && workitems[i].workid == work->id)
		{
			workitems[i].status = 0;
			break;
		}
	}
}
...
inline struct pool *current_pool(void)
{
	struct pool *pool;
...
#endif
#ifdef USE_BITMINE_D12
        OPT_WITH_ARG("--bitmine-d12-options",
                     opt_set_charp, NULL, &opt_bitmine_d12_options,
                     "Bitmine D12 options ref_clk_khz:sys_clk_khz:spi_clk_khz:override_chip_num"),
#endif
#ifdef USE_BITFURY
...
static inline  void calc_midstate(struct work *work)
{
...
static inline int total_work_inc(void)
{
	int ret;
...
static inline struct work *make_work(void)
{
	struct work *work = NULL;

	work = cgcalloc(1, sizeof(struct work));
...
inline void _free_work(struct work **workptr, const char *file, const char *func, const int line)
{
...
static inline int __total_staged(void)
{
	return HASH_COUNT(staged_work);
}

static inline int total_staged(void)
{
	int ret;
...
static inline void suffix_string(uint64_t val, char *buf, size_t bufsiz, int sigdigits)
{
...
double cgpu_runtime(struct cgpu_info *cgpu)
{
	...
	if (cgpu->dev_start_tv.tv_sec == 0)
		...
		dev_runtime = tdiffex(&now, &(cgpu->dev_start_tv));
	}
...
double tsince_restart(void)
{
	...
	dbvalue = tdiffex(&now, &restart_tv_start);
...
double tsince_update(void)
{
	...
	dbvalue = tdiffex(&now, &update_tv_start);
	     ...
	     char *hashshow, bool resubmit, char *worktime)
{
	...
	if (json_is_true(res) || (work->gbt && json_is_null(res))) {
		...
		if (!QUIET) {
			if (total_pools > 1)
				applog(LOG_NOTICE, "Accepted %s %s %d(%s) pool %d %s%s",
				       hashshow, cgpu->drv->name, cgpu->device_id,work->ttydev_name, work->pool->pool_no, resubmit ? "(resubmit)" : "", worktime);
			else
...
static inline void show_hash(struct work *work, char *hashshow)
{
...
static inline bool submit_upstream_work(struct work *work, CURL *curl, bool resubmit)
{
	json_t *val, *res, *err;
	char *s;
	bool rc = false;
	...
	if (!QUIET) {
		...
		if (opt_worktime) {
			...
			/*
			double getwork_time = tdiff((struct timeval *)&(work->tv_getwork_reply),(struct timeval *)&(work->tv_getwork));
			double getwork_to_work = tdiff((struct timeval *)&(work->tv_work_start),(struct timeval *)&(work->tv_getwork_reply));
			double work_time = tdiff((struct timeval *)&(work->tv_work_found),(struct timeval *)&(work->tv_work_start));
			double work_to_submit = tdiff(&tv_submit,(struct timeval *)&(work->tv_work_found));
			double submit_time = tdiff(&tv_submit_reply, &tv_submit);
			*/
			double getwork_time = tdiffex((struct timeval *)&(work->tv_getwork_reply),(struct timeval *)&(work->tv_getwork));
			double getwork_to_work = tdiffex((struct timeval *)&(work->tv_work_start),(struct timeval *)&(work->tv_getwork_reply));
			double work_time = tdiffex((struct timeval *)&(work->tv_work_found),(struct timeval *)&(work->tv_work_start));
			double work_to_submit = tdiffex(&tv_submit,(struct timeval *)&(work->tv_work_found));
			double submit_time = tdiffex(&tv_submit_reply, &tv_submit);
			...
			if (work->clone) {
				...
				//tdiff((struct timeval *)&(work->tv_cloned),(struct timeval *)&(work->tv_getwork_reply)));
				tdiffex((struct timeval *)&(work->tv_cloned),(struct timeval *)&(work->tv_getwork_reply)));
			}
			else
				strcpy(workclone, "O");
...
static inline bool pool_unusable(struct pool *pool)
{
	if (pool->idle)
		return true;
	if (pool->enabled != POOL_ENABLED)
...
static inline double le256todouble(const void *target)
{
	uint64_t *data64;
	double dcut64;
...
static inline double diff_from_target(void *target)
{
	double d64, dcut64;
 ...
 */
static inline void calc_diff(struct work *work, double known)
{
...
inline static void benchfile_dspwork(struct work *work, uint32_t nonce)
{
	char buf[1024];
	uint32_t dn;
	int i;
...
static inline void _stage_work(struct work *work);
...
static inline void _copy_work(struct work *work, const struct work *base_work, int noffset)
{
	uint32_t id = work->id;
...
inline struct work *copy_work_noffset(struct work *base_work, int noffset)
{
	struct work *work = make_work();
...
static inline bool stale_work(struct work *work, bool share)
{
	struct timeval now;
	time_t work_expiry;
	struct pool *pool;
...
inline uint64_t share_diff(const struct work *work)
{
	bool new_best = false;
	double d64, s64;
	uint64_t ret;
...
static inline void regen_hash(struct work *work)
{
...
static inline struct pool *priority_pool(int choice)
{
...
inline void switch_pools(struct pool *selected)
{
...
inline void _discard_work(struct work **workptr, const char *file, const char *func, const int line)
{
	struct work *work = *workptr;
...
static inline void wake_gws(void)
{
	mutex_lock(stgd_lock);
...
static inline int tv_sort(struct work *worka, struct work *workb)
{
	return worka->tv_staged.tv_sec - workb->tv_staged.tv_sec;
}

static inline bool work_rollable(struct work *work)
{
	return (!work->clone && work->rolltime);
}

static inline bool hash_push(struct work *work)
{
	bool rc = true;
...
inline void remove_pool(struct pool *pool)
{
...
static inline void thread_reportin(struct thr_info *thr)
{
	thr->getwork = false;
	cgtime(&thr->last);
...
static inline void hashmeter(int thr_id, uint64_t hashes_done)
{
	bool showlog = false;
	double tv_tdiff;
	time_t now_t;
...
inline void gettime_us(int64_t *st)
{
	struct timeval tv;
...
static inline void stratum_share_result(json_t *val, json_t *res_val, json_t *err_val,struct stratum_share *sshare)
{
	...
	{		
		...
		gettime_us(&curtime);
		if (d12->accept_time == 0)
		{
			d12->accept_time = curtime;
			//printf("accept_time = %"PRIu64"\n",d12->accept_time);			
		}
		else
		{
			sp = curtime - d12->accept_time;	
			//printf("curtime = %"PRIu64",accept_time = %"PRIu64"sp = %d\n\n",curtime,d12->accept_time,sp);			
			d12->accept_time = curtime;			
		}

		if (d12 != NULL)
		{
			if (sp > 20000000 && sp <= 30000000)
				d12->count = 1;			
...
static inline bool parse_stratum_response(struct pool *pool, char *s)
{
...
inline void clear_pool_work(struct pool *pool)
{
	struct work *work, *tmp;
	int cleared = 0;
...
static void init_stratum_threads(struct pool *pool)
{
    ...
    if (unlikely(pthread_create(&pool->stratum_sthread, &attr, stratum_sthread, (void *)pool)))
        quit(1, "Failed to create stratum sthread");

    param.sched_priority = 70;
    pthread_attr_setschedparam(&attr, &param);	

    if (unlikely(pthread_create(&pool->stratum_rthread, &attr, stratum_rthread, (void *)pool)))
...
retry_stratum:
    if (pool->has_stratum) 
    {
        /* We create the stratum thread for each pool just after
            ...
            */
        bool init = pool_tset(pool, &pool->stratum_init);//1
        if (!init) 
        {
 ...
 * be handled. */
static inline struct work *hash_pop(bool blocking)
{
...
static inline void gen_hash(unsigned char *data, unsigned char *hash, int len)
{
	unsigned char hash1[32];
...
inline void set_target(unsigned char *dest_target, double diff)
{
...
static inline void gen_stratum_work(struct pool *pool, struct work *work)
{
    ...
    if (opt_debug) {
        ...
        applog(LOG_DEBUG, "Generated stratum merkle %s\n", merkle_hash);
        applog(LOG_DEBUG, "Generated stratum header %s\n", header);
        applog(LOG_DEBUG, "Work job_id %s nonce2 %"PRIu64" ntime %s\n", work->job_id,
...
static void update_gbt_solo(struct pool *pool)
{
	...
	printf("update_gbt_solo make_work()\n");
...
static inline void set_benchmark_work(struct cgpu_info *cgpu, struct work *work)
{
...
inline struct work *get_work(struct thr_info *thr, const int thr_id)
{
    struct cgpu_info *cgpu = thr->cgpu;
    struct work *work = NULL;
    time_t diff_t;
...
static inline void submit_work_async(struct work *work)
{
	struct pool *pool = work->pool;
...
static inline void rebuild_nonce(struct work *work, uint32_t nonce)
{
...
inline bool test_nonce(struct work *work, uint32_t nonce)
{
...
static inline void update_work_stats(struct thr_info *thr, struct work *work)
{
...
inline bool submit_tested_work(struct thr_info *thr, struct work *work)
{
...
static inline bool new_nonce(struct thr_info *thr, uint32_t nonce)
{
	struct cgpu_info *cgpu = thr->cgpu;
...
inline bool submit_nonce(struct thr_info *thr, struct work *work, uint32_t nonce)
{
	int ret = false;
			  ...
			  int noffset)
{
	...
	work->thr = thr;
	work_in->thr = thr;
	printf("submit_noffset_nonce make_work()\n");	
 ...
 * cgpu->queued_work. */
static inline void fill_queue(struct thr_info *mythr, struct cgpu_info *cgpu, struct device_drv *drv, const int thr_id)
{
    do 
    {
        bool need_work;
        ...
        if (need_work) 
        {
            struct work *work = get_work(mythr, thr_id);//hash_pop
            wr_lock(&cgpu->qlock);
...
inline void __add_queued(struct cgpu_info *cgpu, struct work *work)
{
	cgpu->queued_count++;
	HASH_ADD_INT(cgpu->queued_work, id, work);
}

inline struct work *__get_queued(struct cgpu_info *cgpu)
{
	struct work *work = NULL;
...
inline struct work *get_queued(struct cgpu_info *cgpu)
{
	struct work *work;
...
inline void add_queued(struct cgpu_info *cgpu, struct work *work)
{
	wr_lock(&cgpu->qlock);
	__add_queued(cgpu, work);
	wr_unlock(&cgpu->qlock);
...
inline struct work *get_queue_work(struct thr_info *thr, struct cgpu_info *cgpu, int thr_id)
{
...
inline struct work *__find_work_bymidstate(struct work *que, char *midstate, size_t midstatelen, char *data, int offset, size_t datalen)
{
 ...
 * to handle NULL as a return which implies there is no matching work.
 * The common values for midstatelen, offset, datalen are 32, 64, 12 */
inline struct work *find_queued_work_bymidstate(struct cgpu_info *cgpu, char *midstate, size_t midstatelen, char *data, int offset, size_t datalen)
{
	struct work *ret;
...
inline struct work *clone_queued_work_bymidstate(struct cgpu_info *cgpu, char *midstate, size_t midstatelen, char *data, int offset, size_t datalen)
{
 ...
 * to handle NULL as a return which implies there is no matching work.
 * The calling function must lock access to the que if it is required. */
inline struct work *__find_work_byid(struct work *que, uint32_t id)
{
	struct work *work, *tmp, *ret = NULL;
...
inline struct work *find_queued_work_byid(struct cgpu_info *cgpu, uint32_t id)
{
	struct work *ret;
...
inline void __work_completed(struct cgpu_info *cgpu, struct work *work)
{
	cgpu->queued_count--;
...
inline int age_queued_work(struct cgpu_info *cgpu, double secs)
{
	struct work *work, *tmp;
...
inline void work_completed(struct cgpu_info *cgpu, struct work *work)
{
	wr_lock(&cgpu->qlock);
	__work_completed(cgpu, work);
	wr_unlock(&cgpu->qlock);
...
inline struct work *take_queued_work_bymidstate(struct cgpu_info *cgpu, char *midstate, size_t midstatelen, char *data, int offset, size_t datalen)
{
	struct work *work;
...
inline void flush_queue(struct cgpu_info *cgpu)
{
	struct work *work = NULL;
 ...
 * directly. */
inline void hash_queued_work(struct thr_info *mythr)
{
...
static void convert_to_work(json_t *val, int rolltime, struct pool *pool, struct timeval *tv_lp, struct timeval *tv_lp_reply)
{
	...
	work = make_work();
	printf("convert_to_work make_work()\n");
...
# ifdef __linux
    ...
    opt_kernel_path = alloca(PATH_MAX);//4096
    strcpy(opt_kernel_path, CGMINER_PREFIX);//usr/local/bin
    ...
    initSetting(CONFIG_FILE_PATH);
    for (i = 0; i < 3; i++)
    {
        if (setting.pool[i].url)
        {
            set_user(setting.pool[i].user);
            set_pass(setting.pool[i].pass);
            set_url(setting.pool[i].url);
        }
    }

    applog(LOG_DEBUG, "pool0:%s", setting.pool[0].url);
    applog(LOG_DEBUG, "pool1:%s", setting.pool[1].url);
    applog(LOG_DEBUG, "pool2:%s", setting.pool[2].url);
    opt_api_listen = setting.api_listen;
    opt_api_allow  = setting.api_allow;
    opt_debug      = setting.isdebug;
    opt_D12Pll     = setting.currentFrequency;
    opt_jobtimeout = getJobTime();
    opt_fail_only  = setting.failover_only;
    opt_submit_stale = setting.no_submit_stale;
    
    applog(LOG_DEBUG, "------ChipNum=%d------\n", setting.chipNumber);
    ...
    applog(LOG_NOTICE, "Started %s", packagename);
    ...
    if (cnfbuf) {
        ...
        {
            case 0:
                applog(LOG_ERR, "Fatal JSON error in configuration file.");
                applog(LOG_ERR, "Configuration file could not be used.");
                break;
            case -1:
                applog(LOG_ERR, "Error in configuration file, partially loaded.");
                if (use_curses)
                    applog(LOG_ERR, "Start cgminer with -T to see what failed to load.");
                break;
            default:
                break;
        }
    ...
    if (opt_display_devs) 
    {
        applog(LOG_INFO, "Devices detected:");
        for (i = 0; i < total_devices; ++i) 
        {
            struct cgpu_info *cgpu = devices[i];
            if (cgpu->name)
                applog(LOG_INFO, " %2d. %s %d: %s (driver: %s)", i, cgpu->drv->name, cgpu->device_id, cgpu->name, cgpu->drv->dname);
            else
                applog(LOG_INFO, " %2d. %s %d (driver: %s)", i, cgpu->drv->name, cgpu->device_id, cgpu->drv->dname);
        }
    ...
    applog(LOG_DEBUG, "mining_threads = %d\n",mining_threads);
    ...
    // Start mine threads for all devices
    k = 0;
    for (i = 0; i < total_devices; ++i) 
    {
        struct cgpu_info *cgpu = devices[i];
    ...
    while (!pools_active) 
    {
        ...
        if (!use_curses)
        {
            ...
            struct tty_chain *d12;
            struct cgpu_info *cgpu;
            applog(LOG_ERR,"disable all core!\n");
            for (i = 0; i < total_devices; ++i) 
            {
                cgpu = devices[i];
                d12   = cgpu->device_data;
