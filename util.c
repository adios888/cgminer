...
inline void *_cgcalloc(const size_t memb, size_t size, const char *file, const char *func, const int line)
{
	void *ret;
...
inline void __bin2hex(char *s, const unsigned char *p, size_t len)
{
	int i;
 ...
 * aligned array  sizes */
inline char *bin2hex(const unsigned char *p, size_t len)
{
	ssize_t slen;
	char *s;
...
inline void cgtimer_time(cgtimer_t *ts_start)
{
...
inline void cgsleep_us(int64_t us)
{
	cgtimer_t ts_start;
...
inline int ms_tdiff(struct timeval *end, struct timeval *start)
{
...
int64_t tdiffex(struct timeval *end, struct timeval *start)
{
	return (end->tv_sec - start->tv_sec)*1000000 + (end->tv_usec - start->tv_usec);
}
...
bool auth_stratum(struct pool *pool)
{
	...
	bool ret = false;
        ...
        applog(LOG_DEBUG,"%s swork_id=%d rpc_user=%s rpc_pass=%s\n",
            __func__,swork_id, pool->rpc_user,pool->rpc_pass);
...
#define DEBUG_MEM_SIZE8192    4096
#define DEBUG_ITEM_SIZE       64
#define  DEBUG_ITEMS          (DEBUG_MEM_SIZE8192 / DEBUG_ITEM_SIZE)
static pthread_mutex_t dbg_msg_mutex = PTHREAD_MUTEX_INITIALIZER;
typedef struct
{
    int     current_row;        //count from 0 to DEBUG_ITEMS-1
    uint8_t msg_buffer[DEBUG_ITEMS][DEBUG_ITEM_SIZE];
} msg_mmu;
static msg_mmu mm;
/* 1..DEBUG_ITEM_SIZE will eat one line */
#define STRING_TRAILING_ZERO_LEN  1
#define MSG_EAT_LINES(sz) (((sz) + STRING_TRAILING_ZERO_LEN - 1) /DEBUG_ITEM_SIZE + 1)

static bool make_room(size_t sz)
{
    int x_rows = (sz + 1 - 1) / DEBUG_ITEM_SIZE;//sz + trailing \0 -1
    //sz data needs x + 1 array item to hold it
    if ((x_rows + 1) > DEBUG_ITEMS)
    {
        printf("too much data to be hold\n");
        return false;
    }
    int sum_rows = mm.current_row + x_rows + 1;
    int sum_rows_minus_1 = sum_rows - 1;
    if (sum_rows_minus_1 / DEBUG_ITEMS)//will not exceed 1 <=1
    {
        int need_eat_minimum_lines = sum_rows - DEBUG_ITEMS;
        int i = 0;
        //but we need to remove the qutoine number items, instead lines
        // each item eats at least one line
        int eat_lines = 0;
        while (i < need_eat_minimum_lines)
        {
            int item_size = strlen(mm.msg_buffer[eat_lines]);
            eat_lines += (item_size) / DEBUG_ITEM_SIZE + 1;
            i++;
        }

        memmove((void *) &mm.msg_buffer[0][0], (void *) &mm.msg_buffer[eat_lines ][0],
                DEBUG_ITEM_SIZE * (mm.current_row - eat_lines));
        mm.current_row -= eat_lines;
    }
    return true;
}


bool debug_buf_write(char *dbgbuf, size_t sz /*sz exclude 0*/)
{
    int    i               = ((sz + 1 - 1) / DEBUG_ITEM_SIZE);
    size_t total_copy_size = 0;
    bool   ret             = false;

    pthread_mutex_lock(&dbg_msg_mutex);
    do
    {
        if (make_room(sz) == false)
            break;
        while (i)
        {
            memcpy((void *) &mm.msg_buffer[mm.current_row][0], dbgbuf + total_copy_size,
                   DEBUG_ITEM_SIZE);
            total_copy_size += DEBUG_ITEM_SIZE;
            i--;
            mm.current_row++;
        }
        memcpy((void *) &mm.msg_buffer[mm.current_row][0], dbgbuf + total_copy_size,
               sz - total_copy_size + 1);
        mm.current_row++;
        ret = true;
    } while (0);
    pthread_mutex_unlock(&dbg_msg_mutex);
    return ret;
}


bool debug_buf_read(char *buf, size_t sz)
{
    int i = 0, pos = 0;
    if (mm.current_row == 0)
    {
    ...
    while (sz)
    {
        size_t item_size = strlen((void *) &mm.msg_buffer[i]);
        size_t to_read   = (item_size > sz) ? sz  : item_size;
        memcpy(buf + pos, (void *) &mm.msg_buffer[i][0], to_read);
        sz  -= to_read;
        pos += to_read;
        i   += ((item_size + 1 - 1) / DEBUG_ITEM_SIZE) + 1;
        if (i >= mm.current_row)
        {
            break;
        }
...
inline void RenameThread(const char* name)
{
	char buf[16];
...
#ifdef __APPLE__
inline void _cgsem_init(cgsem_t *cgsem, const char *file, const char *func, const int line)
{
	int flags, fd, i;
...
inline void _cgsem_post(cgsem_t *cgsem, const char *file, const char *func, const int line)
{
	const char buf = 1;
	int ret;
...
inline void _cgsem_wait(cgsem_t *cgsem, const char *file, const char *func, const int line)
{
	char buf;
	int ret;
retry:
...
inline void cgsem_destroy(cgsem_t *cgsem)
{
...
inline int _cgsem_mswait(cgsem_t *cgsem, int ms, const char *file, const char *func, const int line)
{
	struct timeval timeout;
	int ret, fd;
	fd_set rd;
...
inline void cgsem_reset(cgsem_t *cgsem)
{
	int ret, fd;
	fd_set rd;
	char buf;
...
inline void *completion_thread(void *arg)
{
...
extern inline void *completion_thread(void *arg);
inline bool cg_completion_timeout(void *fn, void *fnarg, int timeout)
{
...
inline void _cg_memcpy(void *dest, const void *src, unsigned int n, const char *file, const char *func, const int line)
{
