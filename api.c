...
#include "CgConfig.h"
  ...
  defined(USE_HASHFAST) || defined(USE_BITFURY) || defined(USE_BLOCKERUPTER) || defined(USE_KLONDIKE) || \
	...
	defined(USE_MINION) || defined(USE_COINTERRA) || defined(USE_BITMINE_A1) || defined(USE_BITMINE_D12S) || \
...
extern void reset_param(const char* path);
#if defined WIN32
...
#define _LCD		"LCD"
#define _RESTART        "RESTART"
#define _RESET_MCU      "RESET_MCU"
#define _DBGOUTPUT      "DBGOUTPUT"
#define _UPDATE_CONFIG  "UPDATE_CONFIG"
...
#define JSON_RESET_MCU  JSON1 _RESET_MCU JSON1
#define JSON_DBGOUTPUT  JSON1 _DBGOUTPUT JSON2
#define JSON_UPDATE_CONFIG  JSON1 _UPDATE_CONFIG JSON1
...
//jason.fang
#define MSG_DBGOUTPUT 125
#define MSG_GETELECTRON 127


static char watchbuf[2048];
static char use_special_format = 0;
...
static void summary(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	root = api_add_uint(root, "temper1", &(tempera_val[0]), false);
	root = api_add_uint(root, "temper2", &(tempera_val[1]), false);
	root = api_add_uint(root, "temper3", &(tempera_val[2]), false);


	root = api_add_volts(root, "inputVolt1", &input_volt_val[0], false);
	root = api_add_volts(root, "inputVolt2", &input_volt_val[1], false);
	root = api_add_volts(root, "inputVolt3", &input_volt_val[2], false);

	//printf("%f, %f, %f\n", input_volt_val[0], input_volt_val[1], input_volt_val[2]);

	root = api_add_volts(root, "inputCurrent1", &input_current_val[0], false);
	root = api_add_volts(root, "inputCurrent2", &input_current_val[1], false);
	root = api_add_volts(root, "inputCurrent3", &input_current_val[2], false);


	root = api_add_volts(root, "outputVolt1", &output_volt_val[0], false);
	root = api_add_volts(root, "outputVolt2", &output_volt_val[1], false);
	root = api_add_volts(root, "outputVolt3", &output_volt_val[2], false);
...
static void watch(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	int dev=0;
	int asc=0;
	char *status=NULL;
	...
	for(asc=0; asc<numasc; asc++)
	{
		...
		dev_runtime = cgpu_runtime(cgpu);
		status = (char *)status2str(cgpu->status);
		double mhs = cgpu->total_mhashes / dev_runtime;//average

		if (0 == asc)
		{
			sprintf(watchbuf, "when:%d diff:%d boards:", when, cgpu->last_share_diff);
		}

		memset(tmpbuf, 0, 1024);	
		sprintf(tmpbuf, "%s_%0.2f_%0.2f_%d_%d_%d_%0.2f_%0.2f|",status, cgpu->rolling, mhs, cgpu->rejected, cgpu->accepted, tempera_val[asc],input_current_val[asc], output_volt_val[asc]);
	
		strcat(watchbuf, tmpbuf);	
	}

	ptr = strrchr(watchbuf, '|');
	*ptr= ' ';
//	printf("\n%s\n", watchbuf);

	strcat(watchbuf, "pools:");	
	...
	for (i = 0; i < total_pools; i++) 
	{
		...
		memset(tmpbuf, 0, 1024);
		sprintf(tmpbuf, "%s_%s_%s_%lld_%lld|", status,pool->rpc_url, pool->rpc_user, pool->rejected, pool->accepted);
		strcat(watchbuf,tmpbuf);
		
		//printf("%s_%s_%s_%d_%d\n", status,pool->rpc_url, pool->rpc_user, pool->rejected, pool->accepted);
	}

	ptr = strrchr(watchbuf, '|');
	*ptr='\0';
	
	use_special_format = 1;
...
#if 1
static void dbgoutput ( struct io_data *io_data, __maybe_unused SOCKETTYPE c,
                       __maybe_unused char *param, bool isjson, __maybe_unused char group )
{
    ...
    memset(dbgbuf, 0, 4096);
    message ( io_data, MSG_DBGOUTPUT, 0, NULL, isjson );
    io_open = io_add ( io_data, isjson ? COMSTR JSON_DBGOUTPUT : _DBGOUTPUT COMSTR );

    if (debug_buf_read(dbgbuf, 4095) == false)
        strcpy(dbgbuf, "");
    char *p = escape_string(dbgbuf, isjson);
    {
        root = api_add_escape( root, "DBGOUTPUT", p, false);
    }
...
struct CMDS {
	...
	{ "lockstats",		lockstats,	true,	true },
        { "dbgoutput",          dbgoutput,      true,   false },
